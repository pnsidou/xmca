...
try:
    ...
    # If environment variable DASK_SCHEDULER is set, the corresponding
    # scheduler file will be used
    scheduler_file = os.environ.get('DASK_SCHEDULER')
    client = Client(scheduler_file=scheduler_file, timeout=5)
except OSError as e:
    # In case scheduler from scheduler_file times out, try with a LocalCluster
    client = Client()
